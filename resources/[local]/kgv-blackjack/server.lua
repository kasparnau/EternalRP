...
function SetPlayerBet(i, seat, bet, betId, double, split)
	...
	if num ~= nil then
		if double == false and split == false then
			...
			if source ~= nil and players[i][num] ~= nil and FindPlayerIdx(players[i], source) ~= nil then
										...
										while v.player_in == true and #v.hand < 5 do
											...
											if tableTracker[tostring(v.player)] == nil then
												...
												elseif move == "split" then
													if not TakeMoney(v.player, v.bet) then v.player_in = false break end 
													v.bet = v.bet*2
...
exports["kgv-blackjack"]:SetGetChipsCallback(function(source)
	return exports['inventory']:amountOfItem(source, 'casino-chips')
end)

exports["kgv-blackjack"]:SetTakeChipsCallback(function(source, amount)
	local success = exports['inventory']:removeItem(source, 'casino-chips', amount)
	if not success then
		local character = exports['players']:getCharacter(source)
		print (("^5[Blackjack]^7 ^3[WARN]^7 | CID: %s | Duplicated %s chips, kicking"):format(character.cid, amount))
		DropPlayer(source, "Chips Mismatch")
	end
end)

exports["kgv-blackjack"]:SetGiveChipsCallback(function(source, amount)
	local success =  exports['inventory']:addItem(source, 'casino-chips', amount)
	if not success then
		local character = exports['players']:getCharacter(source)
		print ("^5[Blackjack]^7 ^3[WARN]^7 | CID: %s | Failed to add %s chips"):format(character.cid, amount)
	end
end)

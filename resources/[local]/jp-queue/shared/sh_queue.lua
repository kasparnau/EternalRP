...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() and false == true then
        -- let them in the server
        local added = Queue:AddToConnecting(ids, true, true, done)
        if not added then CancelEvent() return end
    ...
    while true do
        ...
        local msg = (string_format(Config.Language.pos .. qTime, pos, Queue:GetSize(), "")..priorityText)
        update(msg, data.deferrals)
    end
end
...
if Config.DisableHardCap then
    Queue:DebugPrint("^1 [jp-queue] Disabling hardcap ^7")

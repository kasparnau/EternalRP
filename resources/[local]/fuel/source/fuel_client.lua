...
function ManageFuelUsage(vehicle)
	if not Entity(vehicle).state[Config.FuelDecor] then
...
Citizen.CreateThread(function()
	while true do
		...
		if pumpDistance < 2.5 then
			...
			currentCash = exports['inventory']:amountOfItem("cash")
		else
			isNearPump = false
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		local oldFuel = Entity(vehicle).state[Config.FuelDecor]
...
RegisterCommand("fuel", function(source, args, raw)
	...
	if IsPedInAnyVehicle(ped, false) then
		...
		exports['fuel']:SetFuel(entity, 100)
	end
end)
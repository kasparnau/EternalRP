...
Citizen.CreateThread(function()
	...
	if not file or #GetPlayers() == 0 then
		file = io.open(path, 'a')
...
AddEventHandler('nui_doorlock:updateState', function(doorID, locked, src, usedLockpick)
	...
	local character = exports['players']:getCharacter(playerId)

	if type(doorID) ~= 'number' then
		print(('nui_doorlock: %s (%s) didn\'t send a number! (Sent %s)'):format(character.first_name, character.cid, doorID))
		return
	end

	if type(locked) ~= 'boolean' then
		print(('nui_doorlock: %s (%s) attempted to update invalid state! (Sent %s)'):format(character.first_name, character.cid, locked))
		return
	end

	if not Config.DoorList[doorID] then
		print(('nui_doorlock: %s (%s) attempted to update invalid door! (Sent %s)'):format(character.first_name, character.cid, doorID))
		return
	end
	
	if not IsAuthorized(character, Config.DoorList[doorID], doorInfo[doorID], usedLockpick) then
		return
	end
...
events:RegisterServerCallback('nui_doorlock:getDoorInfo', function(source, cb)
	cb(doorInfo)
end)

function IsAuthorized(character, doorID, locked, usedLockpick)
	local playerFaction = character.faction

	local canOpen = false

	if playerFaction and doorID.factions then
		for name,level in pairs(doorID.factions) do
			if (playerFaction.group.faction_name == name and playerFaction.member.rank_level >= level) then
				canOpen = true
				break
			end
		end
		if not canOpen and not doorID.items then
			print(('nui_doorlock: %s (%s) was not authorized to open a locked door!'):format(character.first_name, character.cid))
		end
	end

	-- if not canOpen and doorID.items then
	-- 	for i,v in pairs (doorID.items) do
	-- 		if exports['inventory']:hasEnoughOfItem(v, 1) then
	-- 			exports['inventory']:removeItem(v, 1)
	-- 			canOpen = true
	-- 			break
	-- 		end
	-- 	end
...
AddEventHandler('nui_doorlock:newDoorCreate', function(model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, doubleDoor, doorname)
	...
	if not IsPlayerAceAllowed(source, 'command.newdoor') then print(character.first_name.. 'attempted to create a new door but does not have permission') return end
	...
	file = io.open(path, 'a+')
	if not doorname then label = '\n\n-- UNNAMED DOOR CREATED BY '..character.first_name..'\ntable.insert(Config.DoorList, {'
	else
		label = '\n\n-- '..doorname.. '\ntable.insert(Config.DoorList, {'
	end
	file:write(label)

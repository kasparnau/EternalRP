...
Citizen.CreateThread(function()
	...
	Citizen.Wait(1000)
	events:Trigger('nui_doorlock:getDoorInfo', function(doorInfo)
					...
					for k, v in pairs(Config.DoorList[doorID].doors) do
						...
						if Config.DoorList[doorID].slides then
							if Config.DoorList[doorID].locked then
								...
								DoorSystemSetAutomaticDistance(v.doorHash, 30.0, false, true)
								if k == 2 then playSound(Config.DoorList[doorID], src) return end -- End the loop
							end
...
local lasttext, isDrawing, wasShowing
function Draw3dNUI(text)
	local paused = false
	if IsPauseMenuActive() then paused = true end
	-- if text ~= lasttext or paused then
	isDrawing = true
	if paused then
		if wasShowing then
			wasShowing = false
		...
		if not wasShowing or (wasShowing and lasttext ~= text) then
			wasShowing = true
			local col
			if text == 'Lukustatud' or text == 'Lukustab' then
				col = 'red'
			else
				col = 'default'
			end
...
function dooranim()
	...
	TaskPlayAnim(playerPed, "anim@heists@keycard@", "exit", 8.0, 1.0, -1, 49, 0, 0, 0, 0)
    Citizen.Wait(550)
...
RegisterCommand('+doorInteract', function()
...
RegisterCommand('-doorInteract', function() end, false)
exports['jp-keybinds']:registerKeyMapping("InteractDoor", "Doors", "Interact With Door", "+doorInteract", "-doorInteract", "E")
...
-- local printEntityDetails = true
-- AddEventHandler("target:changed", function(pEntity, pEntityType, pEntityCoords)	
...
-- 		local flags = exports["jp-flags"]:GetPedFlags(pEntity)

...
window.addEventListener('message', function(event) {
    ...
    if (item.response == 'openTarget') {
        ...
        $.each(item.data, function (index, item) {
            ...
            $("#target-"+index).data('TargetParams', item.params);
...
$(document).on('mousedown', (event) => {
    ...
    if (element.id.split("-")[0] === 'target') {
        ...
        let TargetParams = $("#"+element.id).data('TargetParams');

        $.post('https://jp-interact/selectTarget', JSON.stringify({
            event: TargetData,
            params: TargetParams
        }));
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            $(".target-label").html("");
            $('.target-wrapper').hide();
            $.post('https://jp-interact/closeTarget');
            break;
    }
});

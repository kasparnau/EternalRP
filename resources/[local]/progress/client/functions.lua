...
exports('doingAction', function() return isDoingAction end)
...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or mythic_action.useWhileDead then
        if not isDoingAction then
            ...
            Citizen.CreateThread(function ()
                ...
                while isDoingAction do
                    ...
                    if IsControlJustPressed(0, 178) and mythic_action.canCancel then
                        TriggerEvent("progress:client:cancel")
                    end
            ...
            exports['alerts']:notify('Tasks', "Already doing an action", "errorAlert", 1000)
        end
    else
        exports['alerts']:notify('Tasks', "Cannot perform an action when dead", "errorAlert", 1000)
    end
end

...
AddEventHandler("onClientResourceStart", function(resName) -- Initialises the script, sets up voice range, voice targets and request sync with server
	...
	exports['status']:setVoiceColor('#ac5656')
...
Citizen.CreateThread(function()
	while true do
		if initialised then
			...
			if lastUpdatedTalking ~= playerTalking then
				lastUpdatedTalking = playerTalking
				exports['status']:setVoiceColor(playerTalking and '#fff67d' or '#ac5656')
			end

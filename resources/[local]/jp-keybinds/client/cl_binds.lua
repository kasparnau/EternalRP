-- Key binds:
-- Usage:
-- exports["np-keybinds-1"]:registerKeyMapping(category, description, onKeyDownCommand, onKeyUpCommand, defaultKey)
...
AddEventHandler("jp-binds:should-execute", function(shouldExecute)
...
exports('registerKeyMapping', function(name, category, description, onKeyDownCommand, onKeyUpCommand, default, event, type)
    ...
    RegisterCommand(cmdStringDown, function()
      ...
      if event then TriggerEvent("jp-binds:keyEvent", name, true) end
      ExecuteCommand(onKeyDownCommand)
    end, false)
    RegisterCommand(cmdStringUp, function()
      if not shouldExecuteBind then return end
      if event then TriggerEvent("jp-binds:keyEvent", name, false) end
